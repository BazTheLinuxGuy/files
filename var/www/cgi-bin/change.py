...
def fetch_records(page_number: int, records_per_page: int) -> list:
	...
	sql = f'SELECT * FROM newfiles ORDER BY fileid LIMIT {
...
def return_html(pageno: int, records: list):
	...
	if DEBUG > 1:
		w('at line 332, myhtml is:\n')

	w(f'\n{myhtml}\n\n')

	myhtml += f'''
<body>
<header>Make changes to or delete a record altogether or cancel</header>
...
def look_for_searchterm(term: str) -> list:
	...
	if not len(lst):
		...
		con.close()
		rv = sorry(searchterm) # calls sys.exit()
		return rv
	if DEBUG > 1:
		w(f'{lst = }\n')
	else:
